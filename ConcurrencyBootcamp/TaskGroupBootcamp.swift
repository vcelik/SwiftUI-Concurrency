...
class TaskGroupBootcampDataManager{
    ...
    func fetchImagesWithTaskGroup() async throws-> [UIImage]{
        ...
        return try await withThrowingTaskGroup(of: UIImage?.self) { group in
            ...
            for urlString in urlStrings{   //if one of the tasks throws an error, application throws an error
                group.addTask {//we can add priority independent from parent task
                    try? await self.fetchImage(urlString: urlString) //returns nil value rather than failing entire app
                }
            }
