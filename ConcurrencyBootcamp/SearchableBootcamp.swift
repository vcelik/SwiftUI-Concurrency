...
final class SearchableViewModel:ObservableObject{
    ...
    enum SearchScopeOption:Hashable{
        case all
        case cuisine(cuisine:CuisineOption)
        ...
        var title:String{
            ...
            case .cuisine(let cuisine):
                return cuisine.rawValue.capitalized
            }
        }
    }
...
//        }
    case .cuisine(let cuisine):
            restaurantsInScope=allRestaurants.filter({$0.cuisine == cuisine})
    }
...
//                SearchScopeOption.cuisine(cuisine: option)
//            })
            
            allSearchScopes=[.all] + allCuisines.map({SearchScopeOption.cuisine(cuisine: $0)})
